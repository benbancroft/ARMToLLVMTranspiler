...
namespace Elf{
    ...
    int readELF(std::string file)
    {
        ...
        ELFHeader header;
        Endianness systemEndianness = htonl(47) == 47 ? Endianness::Big : Endianness::Little;

        char binaryClass;
        binary.read(&binaryClass, 1);
        header.binaryClass = static_cast<BinaryClass>(binaryClass);

        //used from 0x10 onwards.
        char endianness;
        binary.read(&endianness, 1);
        header.endianness = static_cast<Endianness>(endianness);
        ...
        header.binaryEntryPoint = Elf::mapVirtualAddressToFile(header.entryPoint, &header);
        ...
        Arm::ProgramData programData;
        programData.position = binary.tellg();
        Arm::HandleInstruction(&programData, systemEndianness, &binary, &header);
